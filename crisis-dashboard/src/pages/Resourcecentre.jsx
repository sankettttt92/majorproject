...
//   // delete
//   const [deleting, setDeleting] = useState(null); // holds team id being deleted
...
const STATUS_TONE = {
  AVAILABLE: "green", DEPLOYED: "blue", MAINTENANCE: "muted", OFFLINE: "red", COMPLETED: "purple",
};
const ALL_STATUSES = ["AVAILABLE", "DEPLOYED", "MAINTENANCE", "OFFLINE", "COMPLETED"];
...
export default function ResourceCenter() {
  ...
  const [deletingFromDialog, setDeletingFromDialog] = useState(false);
  ...
  function openStatusDialog(team) {
    setStatusTeam(team);
  ...
  async function handleSaveStatus() {
    ...
    setStatusError(null);
    try {
      await api.updateTeamStatus(statusTeam.id, statusValue);
      refetchTeams();
      setStatusTeam((prev) => (prev ? { ...prev, status: statusValue } : prev));
    } catch (err) {
  ...
  async function handleDeleteFromDialog() {
    if (!statusTeam || deletingFromDialog) return;
    setDeletingFromDialog(true);
    setStatusError(null);
    try {
      await api.deleteTeam(statusTeam.id);
      refetchTeams();
      closeStatusDialog();
    } catch (err) {
      if (err.status === 404) {
        setStatusError("This team was already deleted. Refreshing list…");
        refetchTeams();
        closeStatusDialog();
      } else if (err.status === 409) {
        setStatusError(err.data || "Only COMPLETED teams can be deleted.");
      } else {
        setStatusError("Couldn't delete this team. Try again.");
      }
    } finally {
      setDeletingFromDialog(false);
    }
  }
  ...
  return (
    <div className="page">
            ...
            <div className="table-card" key={cat}>
              ...
              <table className="data-table">
                ...
                <tbody>
                  ...
                  {items.map((t) => (
                    <tr key={t.id} className="data-row" onClick={() => openStatusDialog(t)} style={{ cursor: "pointer" }}>
                      <td className="mono link-cell">{t.id}</td>
                      <td>{t.name}</td>
                      <td><span className="pill pill--purple">{ORG_LABEL[t.org_type] || t.org_type}</span></td>
                      <td><span className={`pill pill--${STATUS_TONE[t.status] || "muted"}`}>{t.status}</span></td>
                      ...
                      <td>
                        <button
                          className="btn-secondary btn-small"
                          disabled={t.status !== "AVAILABLE"}
                          onClick={(e) => { e.stopPropagation(); setAllocateTarget(t); }}
                        >
                          Assign
                        </button>
                      </td>
      ...
      {statusTeam && (
        <div className="drawer-backdrop" onClick={closeStatusDialog}>
          <div className="drawer" onClick={(e) => e.stopPropagation()}>
            <div className="drawer-header">
              <span className="mono">{statusTeam.id}</span>
              <button onClick={closeStatusDialog} className="drawer-close"><X size={16} /></button>
            </div>
            <h2>{statusTeam.name}</h2>
            <p className="text-secondary">{ORG_LABEL[statusTeam.org_type] || statusTeam.org_type} · {statusTeam.base || "Base unknown"}</p>
            <h3>Status</h3>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button className="btn-secondary btn-small" onClick={handleSaveStatus} disabled={savingStatus || statusValue === statusTeam.status}>
                {savingStatus ? "Saving…" : "Save status"}
              </button>
              <button
                className="btn-secondary btn-small"
                onClick={handleDeleteFromDialog}
                disabled={statusTeam.status !== "COMPLETED" || deletingFromDialog}
                title={statusTeam.status !== "COMPLETED" ? "Mark the team COMPLETED to enable delete" : ""}
              >
                {deletingFromDialog ? "Deleting…" : "Delete team"}
              </button>
            </div>
          </div>
        </div>
