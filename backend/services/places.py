...
# Multiple mirrors tried in order — Overpass's main instance frequently
# returns 504 Gateway Timeout under load. This is called server-side now
# (no CORS concern here, unlike the old direct-from-browser calls), so
# falling back through mirrors is safe and cheap.
OVERPASS_URLS = [
...
async def fetch_nearby_facilities(
    ...
    Tries multiple Overpass mirrors in order (see OVERPASS_URLS) since the
    main public instance frequently 504s under load. If every mirror fails,
    returns an empty list instead of raising — callers (routers/resources.py)
    already treat an empty list as "no facilities found" and the frontend
    shows a friendly message rather than a hard error.
    """
    amenity_filter = "|".join(PLACE_TYPE_MAP.keys())
    query = f"""
[out:json][timeout:10];
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        for url in OVERPASS_URLS:
            try:
                ...
                # Covers timeouts, connection errors, and non-2xx responses
                # (e.g. 504 Gateway Timeout). Try the next mirror.
                continue

    if data is None:
        # All mirrors failed — degrade gracefully instead of a 500.
        return []
