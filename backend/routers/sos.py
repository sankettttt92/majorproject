...
# v5 fix: check_identity now queries the real `register` table (models/register.py)
# instead of the old unused `models/user.py`. Register has no last-known-location
# columns yet, so the location-spoofing check against a prior known position is
# disabled for now (last_lat/last_lon/last_seen are always None). Revisit if
# Register ever gains location tracking.
# """
# from fastapi import APIRouter, Depends, BackgroundTasks, HTTPException
# from sqlalchemy.exc import IntegrityError
# from sqlalchemy.ext.asyncio import AsyncSession
...
#     # Register has no last-known-location tracking yet — spoofing check
#     # against last known position is effectively disabled until that's added.
#     last_lat = None
#     last_lon = None
#     last_seen = None
