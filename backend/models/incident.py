...
class Incident(Base):
    ...
    # Who sent it — real FK to register.id now
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("register.id"), index=True, nullable=False
    )
    device_id: Mapped[str] = mapped_column(String(128), nullable=True)
    ...
    accuracy_meters: Mapped[float] = mapped_column(Float, nullable=True)
    address: Mapped[str] = mapped_column(Text, nullable=True)
    zone: Mapped[str] = mapped_column(String(128), nullable=True)
    ...
    user = relationship("Register", backref="incidents")